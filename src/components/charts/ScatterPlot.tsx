...
export default memo(function ScatterPlot({
  ...
  if (trendLine && points.length > 2) {
    ...
    const ssRes = points.reduce(
      (a, p) => a + (p.y - (slope * (p.x as number) + intercept)) ** 2,
      0,
    );
    ...
    <div style={{ marginBottom: 6 }}>
      ...
      >
        ...
        {Array.from({ length: 5 }, (_, i) => xMin + (xRng * i) / 4).map((v, i) => (
          <text key={i} x={px(v)} y={height - 8} fontSize={8} fill="#94a3b8" textAnchor="middle">
            {v.toFixed(0)}
          </text>
        ))}
        ...
        {points.map((p, i) => (
          ...
          >
            <title>
              {xLabel}={(p.x as number).toFixed(0)}, {yLabel}={p.y.toFixed(3)}
              {p.label ? ' · ' + p.label : ''}
            </title>
          </circle>
        ))}
