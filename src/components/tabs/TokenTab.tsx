...
export default function TokenTab({ weights, changes, segments }: TokenTabProps) {
  ...
  const changesMap = useMemo<Map<number, TokenChange>>(() => {
    ...
    (changes || []).forEach((c) => {
      if (c.token_index != null) m.set(c.token_index, c);
    });
    return m;
  }, [changes]);
  ...
  const filteredTokens = useMemo<number[]>(() => {
    ...
    if (fSource !== 'all')
      indices = indices.filter((i) => {
        const ri = roleMap[i];
        return ri && ri.role === fSource;
      });
    if (sortBy === 'weight_asc') indices.sort((a, b) => weights[a] - weights[b]);
    else if (sortBy === 'weight_desc') indices.sort((a, b) => weights[b] - weights[a]);
    else if (sortBy === 'mult_desc')
      indices.sort((a, b) => {
        const ca = changesMap.get(a),
          cb = changesMap.get(b);
        return (cb?.effective_multiplier ?? 0) - (ca?.effective_multiplier ?? 0);
      });
    return indices;
  ...
  const MAX = 800;
  const tokensActive =
    sortBy !== 'index' || fWtMin !== '' || fWtMax !== '' || fChangesOnly || fSource !== 'all';
  const displayTokens = tokensActive
    ? filteredTokens.slice(0, MAX)
    : weights.map((_, i) => i).slice(0, MAX);
  ...
  const showTt = useCallback(
    (e: React.MouseEvent, i: number, w: number): void => {
      const ri = roleMap[i],
        hasC = changesMap.has(i);
      setTtPos({ x: e.clientX, y: e.clientY });
      setTt({
        type: 'token',
        pos: i,
        w,
        role: ri ? (RV[ri.role] || RV.other).lbl : '?',
        masked: ri ? ri.masked : null,
        hasChange: hasC,
      });
      setHovGrid(i);
    },
  ...
  const legendMatch = (i: number): boolean => {
    ...
    if (legendHover === 'masked')
      return !!ri?.masked && ri?.role !== 'system' && ri?.role !== 'forced';
  ...
  return (
    <div style={{ paddingBottom: 40 }}>
      ...
      <HelpBox>
        Each cell = one token. Color encodes <strong>IS-weight</strong> (importance sampling). Green
        = high weight (≈ on-policy), red = low (off-policy drift). Cells with <strong>⚡</strong>{' '}
        had their weight modified by ESC shaping rules. Click any cell to lock the detail inspector.
        Use filters to find outliers.
      </HelpBox>

      {/* ── Weight Statistics ── */}
      {stats && (
        <Panel title="Weight Statistics" bc={CB.blue}>
        ...
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'flex-end' }}>
          <div>
            ...
            <input
              ...
              step="0.1"
              value={fWtMin}
              onChange={(e) => setFWtMin(e.target.value)}
              placeholder="—"
              style={{
                width: 70,
                padding: '4px 6px',
          ...
          <div>
            ...
            <input
              ...
              step="0.1"
              value={fWtMax}
              onChange={(e) => setFWtMax(e.target.value)}
              placeholder="—"
              style={{
                width: 70,
                padding: '4px 6px',
          ...
          >
            <input
              type="checkbox"
              checked={fChangesOnly}
              onChange={(e) => setFChangesOnly(e.target.checked)}
            />{' '}
            ⚡ Modified only
          </label>
          <div>
      ...
      {weights.length > 0 && (
        <Panel title={`Weight profile (sampled — ${weights.length} tokens)`} bc={CB.teal}>
          <div
            style={{
              height: 60,
              display: 'flex',
      ...
      <Panel title="Interactive token grid" bc={CB.green}>
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
          {(['hybrid', 'proximity', 'role'] as ColorMode[]).map((m) => (
            <button
              key={m}
              onClick={() => setCMode(m)}
              style={{
        ...
        {displayTokens.length >= MAX && (
          ...
          >
            Showing first {MAX} of {tokensActive ? filteredTokens.length : weights.length} tokens.
      ...
      {/* ── Token Inspector ── */}
      {detail && (
        <Panel title={`Token Inspector — #${detail.pos}`} bc={CB.cyan}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            ...
            >
              ...
              >
                ...
                <span style={{ fontWeight: 600, color: '#0f172a', fontFamily: 'var(--font-mono)' }}>
                  {((detail.w / 3.6) * 100).toFixed(1)}%
                </span>
                <span style={{ color: '#94a3b8', fontWeight: 500 }}>Role:</span>
                <span style={{ fontWeight: 600, color: detail.ri ? detail.ri.rv.bg : '#94a3b8' }}>
                  {detail.ri ? detail.ri.rv.lbl : '?'}
                </span>
                <span style={{ color: '#94a3b8', fontWeight: 500 }}>Masked:</span>
                <span style={{ fontWeight: 600, color: detail.ri?.masked ? CB.red : CB.green }}>
                  {detail.ri ? (detail.ri.masked ? '⊘ YES' : '✓ NO') : '?'}
                </span>
            ...
            <div
              style={{
                background: detail.change ? '#fff7ed' : '#f8fafc',
                padding: 10,
                borderRadius: 6,
                border: detail.change ? '1px solid #fed7aa' : '1px solid #e2e8f0',
              }}
            >
              <div
                style={{
                  fontSize: 10,
                  fontWeight: 700,
                  color: detail.change ? CB.orange : '#64748b',
                  marginBottom: 6,
                  textTransform: 'uppercase',
                }}
              >
              ...
              {detail.change ? (
                <React.Fragment>
                  ...
                  >
                    ...
                    <span style={{ fontWeight: 600, fontFamily: 'var(--font-mono)' }}>
                      {detail.change.token_id}
                    </span>
                    <span style={{ color: '#94a3b8', fontWeight: 500 }}>Before:</span>
                    <span
                      style={{ fontWeight: 600, color: CB.red, fontFamily: 'var(--font-mono)' }}
                    >
                      {detail.change.before_weight.toFixed(6)}
                    </span>
                    <span style={{ color: '#94a3b8', fontWeight: 500 }}>After:</span>
                    <span
                      style={{ fontWeight: 600, color: CB.green, fontFamily: 'var(--font-mono)' }}
                    >
                      {detail.change.after_weight.toFixed(6)}
                    </span>
                    <span style={{ color: '#94a3b8', fontWeight: 500 }}>Multiplier:</span>
                    <span
                      style={{ fontWeight: 600, color: CB.yellow, fontFamily: 'var(--font-mono)' }}
                    >
                      {detail.change.multiplier}
                    </span>
                    <span style={{ color: '#94a3b8', fontWeight: 500 }}>Eff. Mult:</span>
                    <span
                      style={{ fontWeight: 600, color: CB.orange, fontFamily: 'var(--font-mono)' }}
                    >
                      {detail.change.effective_multiplier.toFixed(6)}
                    </span>
                    <span style={{ color: '#94a3b8', fontWeight: 500 }}>Type:</span>
                    <span style={{ fontWeight: 600, color: CB.purple }}>
                      {detail.change.change_type || '—'}
                    </span>
                    {detail.change.match_reason && (
                      <>
                        <span style={{ color: '#94a3b8', fontWeight: 500 }}>Reason:</span>
                        <span style={{ fontWeight: 600, color: CB.cyan }}>
                          {detail.change.match_reason}
                        </span>
                      </>
                    )}
                ...
                <div style={{ fontSize: 11, color: '#94a3b8', fontStyle: 'italic' }}>
                  This token was not modified by ESC shaping.
                </div>
              )}
            </div>
          </div>
      ...
      {changes && changes.length > 0 && (
        <Panel title={`Modification Log (${changes.length} changes)`} bc={CB.orange}>
          {/* Grid header */}
          <div
            style={{
              display: 'grid',
            ...
            {changes.map((c, idx) => (
              <div
                ...
                onClick={() =>
                  setDetail({
                    pos: c.token_index,
                    w: weights[c.token_index] ?? 0,
                    change: c,
                    ri: roleMap[c.token_index],
                  })
                }
                style={{
                  display: 'grid',
              ...
              >
                ...
                <span style={{ color: CB.green, fontWeight: 600 }}>
                  {c.after_weight.toFixed(4)}
                </span>
                <span style={{ color: CB.yellow, fontWeight: 600 }}>{c.multiplier}</span>
                <span style={{ color: CB.orange, fontWeight: 600 }}>
                  {c.effective_multiplier.toFixed(4)}
                </span>
