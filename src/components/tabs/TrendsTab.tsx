...
export default function TrendsTab({ rows }: { rows: Rollout[] }) {
  ...
  const ruleStats = useMemo(() => {
    ...
    Object.values(m).forEach((v: any) => {
      v.avgEff = v.count > 0 ? v.effSum / v.count : 0;
    });
  ...
  const viewWinRates = useMemo(() => {
    ...
    Object.values(m).forEach((v: any) => {
      ...
      v.avgReward =
        v.total > 0 ? v.rewards.reduce((a: number, b: number) => a + b, 0) / v.total : 0;
    });
  ...
  const forcedPerStep: ChartPoint[] = byStep
    .filter((s: StepBucket) => s.forced > 0)
    .map((s: StepBucket) => ({
      x: String(s.step),
      y: s.forced / s.total,
    }));

  const klPts: ChartPoint[] = klRewardDual
    .filter((p: KLRewardPoint) => p.kl != null)
    .map((p: KLRewardPoint) => ({ x: p.x, y: p.kl as number }));
  const rwPts: ChartPoint[] = klRewardDual
    .filter((p: KLRewardPoint) => p.rw != null)
    .map((p: KLRewardPoint) => ({ x: p.x, y: p.rw as number }));
  ...
  return (
    <div style={{ paddingBottom: 40 }}>
      {/* 1. HelpBox */}
      <HelpBox>
        <strong>Trends & RL Analytics</strong> — aggregated views across all rollouts. Step-level
        accuracy, reward curves, forced-answer rates, scaffold reliance, KL divergence tracking, and
        per-view win rates. Use these to diagnose training dynamics and spot regressions.
      </HelpBox>
      ...
      {totalForced > 0 && (
        <Panel title="💉 Forced Answer Summary" bc="#f97316">
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 10 }}>
            ...
            <Chip
              label="Forced Rate"
              value={`${(forcedRate * 100).toFixed(1)}%`}
              bg={forcedRate > 0.3 ? '#fef2f2' : '#fff7ed'}
              color={forcedRate > 0.3 ? CB.red : '#c2410c'}
            />
      ...
      <Panel title="📊 Step Aggregates" bc={CB.blue}>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 10 }}>
          ...
          <Chip
            ...
            value={
              rows.length > 0
                ? (rows.reduce((a, r) => a + (r.reward ?? 0), 0) / rows.length).toFixed(3)
                : '—'
            }
            bg="#f0fdf4"
            color={CB.green}
          ...
          <Chip
            ...
            value={
              rows.length > 0
                ? `${((rows.filter((r) => r.correct).length / rows.length) * 100).toFixed(1)}%`
                : '—'
            }
            bg="#f0fdf4"
            color={CB.green}
              ...
              return (
                ...
                >
                  ...
                  <div style={{ fontSize: 11, color: '#334155' }}>
                    {s.total} rows · {(acc * 100).toFixed(0)}% acc
                  </div>
                  <div
                    style={{ fontSize: 10, color: avgR > 0 ? CB.green : CB.red, fontWeight: 600 }}
                  >
                    R̄ {avgR.toFixed(3)}
                  </div>
                  {s.forced > 0 && (
                    <div style={{ fontSize: 9, color: '#c2410c' }}>💉 {s.forced} forced</div>
                  )}
                </div>
              );
            })}
              ...
              return (
                <div key={r.reason} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  >
                    <div
                      style={{
                        ...
                        height: '100%',
                        background:
                          r.totalDelta > 0 ? CB.green : r.totalDelta < 0 ? CB.red : CB.grey,
                        borderRadius: 4,
                  ...
                  <div
                    style={{
                      fontSize: 9,
                      color: r.avgEff > 1 ? CB.green : r.avgEff < 1 ? CB.orange : '#64748b',
                      minWidth: 50,
                      textAlign: 'right' as const,
                      fontFamily: 'var(--font-mono)',
                    }}
      ...
      {scaffoldTrend.length > 1 && (
        <Panel title="🔧 Scaffold Reliance" bc={CB.orange}>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 8 }}>
            <Chip
              label="Start Ratio"
              value={scaffoldTrend[0].y === 999 ? '∞' : scaffoldTrend[0].y.toFixed(2)}
              bg="#fff7ed"
              color={CB.orange}
            />
            <Chip
              label="End Ratio"
              value={
                scaffoldTrend[scaffoldTrend.length - 1].y === 999
                  ? '∞'
                  : scaffoldTrend[scaffoldTrend.length - 1].y.toFixed(2)
              }
              bg="#fff7ed"
              color={CB.orange}
            />
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 8 }}>
            {klRwCorr != null && (
              <>
                <Chip
                  label="Correlation"
                  value={klRwCorr.toFixed(3)}
                  bg={Math.abs(klRwCorr) > 0.7 ? '#fef2f2' : '#f0fdf4'}
                  color={Math.abs(klRwCorr) > 0.7 ? CB.red : CB.green}
                />
                <Chip
                  label="Signal"
                  value={
                    Math.abs(klRwCorr) > 0.7
                      ? '⚠️ HIGH'
                      : Math.abs(klRwCorr) > 0.4
                        ? '⚡ MOD'
                        : '✓ LOW'
                  }
                  bg={Math.abs(klRwCorr) > 0.7 ? '#fef2f2' : '#f0fdf4'}
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {viewWinRates.map((v: ViewWinRate) => (
              ...
              >
                ...
                {v.forced > 0 && (
                  <div style={{ fontSize: 9, color: '#c2410c', marginTop: 2 }}>
                    💉 {v.forced} forced
                  </div>
                )}
              </div>
          ...
          {byView.map((v) => {
            ...
            return (
              <div
                key={v.view}
                style={{
                  background: v.view.includes('naked') ? '#fff7ed' : '#eff6ff',
                  padding: 14,
                  borderRadius: 8,
                  border: `1px solid ${v.view.includes('naked') ? '#fed7aa' : '#bfdbfe'}`,
                  minWidth: 180,
              ...
              >
                <div
                  style={{
                    fontWeight: 800,
                    fontSize: 14,
                    color: v.view.includes('naked') ? CB.orange : CB.blue,
                    marginBottom: 8,
                    textTransform: 'uppercase',
                  }}
                >
                ...
                >
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span>Accuracy:</span>
                    <strong>{Math.round((100 * v.correct) / v.total)}%</strong>
                  </div>
                  <div
                    style={{
                      display: 'flex',
                  ...
                  >
                    <span>Avg R:</span>
                    <span style={{ color: avgR > 0 ? CB.green : CB.red, fontWeight: 700 }}>
                      {avgR.toFixed(3)}
                    </span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span>Avg A:</span>
                    <span style={{ color: avgA > 0 ? CB.green : CB.red, fontWeight: 700 }}>
                      {avgA.toFixed(3)}
                    </span>
                  </div>
                </div>
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              {Object.entries(batchMap)
                ...
                .map(([k, items]) => (
                  ...
                  >
                    <div style={{ fontWeight: 700, fontSize: 13, color: CB.blue, marginBottom: 4 }}>
                      Iter {k}
                    </div>
                    <div style={{ fontSize: 11, color: '#475569', marginBottom: 6 }}>
                      {items.length} rows ·{' '}
                      <span style={{ color: CB.green, fontWeight: 600 }}>
                        {items.filter(({ r }) => r.correct).length}✓
                      </span>
                      {items.length > 0 && (
                        <div style={{ marginTop: 2 }}>
                          ...
                          <strong>
                            {(
                              items.reduce((a, { r }) => a + (r.reward ?? 0), 0) / items.length
                            ).toFixed(3)}
                          </strong>
                        </div>
                      )}
                      ...
                      {items.map(({ r, i }) => (
                        <div
                          ...
                          style={{
                            ...
                            height: 10,
                            borderRadius: 3,
                            background: r.correct ? CB.green : CB.red,
                            boxShadow: 'inset 0 0 0 1px rgba(0,0,0,0.1)',
                          }}
                          title={`Row ${i + 1}: R=${(r.reward ?? 0).toFixed(3)}`}
                        />
                      ))}
                    </div>
                  </div>
