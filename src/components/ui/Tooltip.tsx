...
export default function Tooltip({ data, pos }: TooltipProps) {
  if (!data) return null;
  const left = typeof window !== 'undefined' ? Math.min(pos.x + 14, window.innerWidth - 400) : pos.x + 14;
  const top = typeof window !== 'undefined' ? Math.max(pos.y - 10, 8) : pos.y - 10;

  return (
    <div style={{ position: 'fixed', left, top, background: '#0f172a', color: '#f1f5f9', borderRadius: 8, padding: '12px 16px', fontSize: 11, fontFamily: 'var(--font-mono)', zIndex: 9999, boxShadow: '0 10px 40px rgba(0,0,0,0.5)', border: '1px solid #334155', pointerEvents: 'none', maxWidth: 400, lineHeight: 1.6 }}>
      {data.type === 'segment' && (
        <>
          <div style={{ fontWeight: 700, color: CB.cyan, marginBottom: 6, fontSize: 12, borderBottom: '1px solid #334155', paddingBottom: 4 }}>Segment: {data.tag || '—'}</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '4px 12px' }}>
            <TtRow k="source" v={data.source || '?'} />
            <TtRow k="tokens" v={data.token_count ?? 0} />
            <TtRow k="status" v={data.masked ? '⊘ MASKED' : '✓ TRAINABLE'} vc={data.masked ? CB.red : CB.green} />
            <TtRow k="role" v={(RV[segRole(data as any)] || RV.other).lbl} vc={CB.yellow} />
            {data.text && <TtRow k="preview" v={(data.text || '').substring(0, 100)} vc="#cbd5e1" />}
          </div>
        </>
      )}
      {data.type === 'token' && (
        <>
          <div style={{ fontWeight: 700, color: CB.cyan, marginBottom: 6, fontSize: 12, borderBottom: '1px solid #334155', paddingBottom: 4 }}>Token #{data.pos}</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '4px 12px' }}>
            ...
            {data.masked != null && <TtRow k="training" v={data.masked ? '⊘ NOT USED' : '✓ IN LOSS'} vc={data.masked ? CB.red : CB.green} />}
...
function ChangeTooltip({ c }: { c: TokenChange }) {
  return (
    <>
      <div style={{ fontWeight: 700, color: CB.orange, marginBottom: 6, fontSize: 12, borderBottom: '1px solid #334155', paddingBottom: 4 }}>⚡ Token Modification</div>
      <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '4px 12px' }}>
      ...
      <div style={{ marginTop: 8, padding: '4px 8px', borderRadius: 4, background: '#1e293b', fontSize: 10, color: '#cbd5e1', border: '1px dashed #475569', textAlign: 'center' }}>
